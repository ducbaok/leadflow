 ...
 * Chạy: npm run seed  (cần DATABASE_URL trong .env.local)
 */
import { randomUUID } from 'node:crypto'
import { sql } from 'drizzle-orm'
import { faker } from '@faker-js/faker'
import { getDb } from '../src/db/client'
import { importBatches, leadSources, leads, scoringConfig } from '../src/db/schema'
import { normalizeEmail } from '../src/lib/normalize/email'
import { validatePhone } from '../src/lib/normalize/phone'
import { foldDiacritics, normalizeCompany, normalizePersonName, sortNameTokens } from '../src/lib/normalize/text'
...
async function main() {
  ...
  console.log('→ Sinh 5.000 lead nền...')
  const all: SeedLead[] = []
  ...
  console.log('→ Cài 150 cặp fuzzy dupes chủ đích...')
  ...
  console.log(`→ Insert ${all.length} leads...`)
  ...
  console.log('→ Scoring config mặc định...')
  await db
  ...
  console.log(`✓ Seed xong: ${all.length} leads (${multiSourceCount} lead nhiều nguồn, ${fuzzyClones.length} fuzzy dupes, ${injection.length} injection demo)`)
  process.exit(0)
}
